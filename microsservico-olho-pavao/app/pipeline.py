"""
Pipeline de Dados - Microsserviço Olho de Pavão

Duas responsabilidades:
  1. TREINO: Planilhas → features semanais → dataset para treinar o modelo
  2. PREVISÃO: Dados climáticos do utilizador → mesmas features → previsão
"""
import pandas as pd
import numpy as np
from .config import (
...
def _calcular_fator_temp(temp_media):
    """Fator de temperatura (ótimo 15-20°C para Spilocaea oleaginea)."""
    return np.where(
...
def _calcular_fator_humidade(humidade):
    """Fator de humidade (favorável >= 80%)."""
    return np.where(
        humidade >= 80, 1.0,
...
def preparar_dataset_treino() -> pd.DataFrame:
    """Pipeline completo: planilhas → features semanais → dataset."""
    print("=" * 60)
    print("[Pipeline] Preparando dataset de treino...")
    print("=" * 60)

    # --- Dados de doença ---
    frames = []
    for ficheiro in FICHEIROS_DOENCA:
        df = pd.read_excel(ficheiro, header=1)
    ...
    df_pavao = df_pavao.rename(columns={
        'Data': 'data', 'Repetição': 'repeticao', 'Árvore': 'arvore',
        'Folha': 'folha', 'Visíveis': 'visiveis',
        'Visíveis + Latentes': 'visiveis_latentes',
    })
    ...
    print(f"[Pipeline] Doença: {len(df_pavao)} registos")
    ...
    # --- Dados climáticos ---
    df_clima = pd.read_excel(FICHEIRO_CLIMA, sheet_name="Mirandela")
    ...
    # Agregar clima por semana (médias semanais)
    ...
    df_clima_sem = df_clima_sem[df_clima_sem['ano'] < 2024]
...
def calcular_features_do_input(
    ...
    climáticos enviados pelo utilizador.
    """
