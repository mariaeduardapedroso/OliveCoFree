...
class PrevisaoRequest(BaseModel):
    ...
    temperatura_media_anterior: Optional[float] = Field(
        default=None, description="Temperatura media da semana anterior (C)"
    )
    humidade_anterior: Optional[float] = Field(
        default=None, ge=0, le=100, description="Humidade relativa media da semana anterior (%)"
    )
    precipitacao_anterior: Optional[float] = Field(
        default=None, ge=0, description="Precipitacao media diaria da semana anterior (mm/dia)"
    )
...
class ModeloInfo(BaseModel):
    ...
    # Metricas por janela deslizante (validacao temporal entre anos)
